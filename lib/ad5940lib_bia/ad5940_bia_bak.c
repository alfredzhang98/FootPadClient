...
AppBIACfg_Type AppBIACfg =
    {
        ...
        .DftNum = DFTNUM_256,
        .DftSrc = DFTSRC_ADCRAW,
        .HanWinEn = bTRUE,

        .SweepCfg.SweepEn = bFALSE,
        .SweepCfg.SweepStart = 10000,
        // .SweepCfg.SweepStart = 100000.0,
...
static AD5940Err AppBIASeqCfgGen(void)
{
    ...
    lp_loop.LpAmpCfg.LpTiaSW = LPTIASW(5) | LPTIASW(6) | LPTIASW(7) | LPTIASW(8) | LPTIASW(9) | LPTIASW(12) |
                               LPTIASW(13); /** @todo Optimization needed for new silicon */
    AD5940_LPLoopCfgS(&lp_loop);
...
static AD5940Err AppBIASeqMeasureGen(void)
{
    ...
    clks_cal.DataType = DATATYPE_ADCRAW;
...
static AD5940Err AppBIADataProcess(int32_t *const pData, uint32_t *pDataCount)
{
    ...
    *pDataCount = 0;
    ...
    DataCount = (DataCount / 4) *
                4; /* We expect RCAL data together with Rz data. One DFT result has two data in FIFO, real part and imaginary part.  */
...
AD5940Err AppBIAISR(void *pBuff, uint32_t *pCount)
{
    ...
    if (AD5940_INTCTestFlag(AFEINTC_0, AFEINTSRC_DATAFIFOTHRESH) == bTRUE)
    {
        ...
        FifoCnt = AD5940_FIFOGetCnt(); // yw
        // if(FifoCnt > BuffCount)//yw
        // {
        ...
        // fifo_cfg.FIFOSrc = FIFOSRC_DFT;
        // fifo_cfg.FIFOThresh = 4;//AppIMPCfg.FifoThresh;
        // AD5940_FIFOCfg(&fifo_cfg);
        // fifo_cfg.FIFOEn = bTRUE;
        // AD5940_FIFOCfg(&fifo_cfg);
...
AD5940Err AppfISR(void *pBuff, uint32_t *pCount)
{
    uint32_t BuffCount;
    uint32_t FifoCnt;
    BuffCount = *pCount;
